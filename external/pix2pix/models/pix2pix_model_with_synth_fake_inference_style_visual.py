...
class Pix2PixModel(BaseModel):
    ...
    def get_current_visuals(self):
        ...
        preview_bgr_raw = None
        if self.is_synthetic and self.synthetic_preview_B is not None:
            preview_bgr_raw = self._tensor_to_bgr_u8(self.synthetic_preview_B)
            if preview_bgr_raw is not None:
                preview_bgr = self._overlay_metric_panel(
                    preview_bgr_raw,
                    ["Synthetic preview (visualization only)", "Inference-style rendered reference for this synthetic sample"],
                    anchor="tl",
                )
                visuals["synthetic_preview_B"] = self._bgr_u8_to_tensor_like(preview_bgr, self.synthetic_preview_B)
            else:
                visuals["synthetic_preview_B"] = self.synthetic_preview_B

        fake_val = getattr(self, "fake_B", None)
        if fake_val is not None:
            fake_bgr = self._tensor_to_bgr_u8(fake_val)
            if fake_bgr is not None:
                if self.is_synthetic and preview_bgr_raw is not None:
                    fake_bgr = self._build_synthetic_fake_visual_bgr(fake_bgr, preview_bgr_raw)
