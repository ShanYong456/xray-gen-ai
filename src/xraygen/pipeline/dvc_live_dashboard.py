...
METRIC_FILES = {
    "real_reference": ROOT / "reports/dvc/real_test_mahal_against_train.json",
    "real_ab_reference": ROOT / "results/_gen_real_ab_reference_train_shampoo_or_shampoo_blade/real_ab_input_summary.json",
    "generated_summary": ROOT / "reports/dvc/generated_combo_summary.json",
    "filter_report": ROOT / "reports/dvc/filter_report.json",
    "fid_train": ROOT / "reports/dvc/fid_eval_runs/train/Shampoo_NOBGR_pix2pix_StructCond_V1_Stage24_COMPLETESyn/epoch_latest/metrics.json",
    "fid_test": ROOT / "reports/dvc/fid_eval_runs/test/Shampoo_NOBGR_pix2pix_StructCond_V1_Stage24_COMPLETESyn/epoch_latest/metrics.json",
    "candidate_eval": ROOT / "reports/dvc/candidate_eval.json",
    "production_registry": ROOT / "models/production/model_registry.json",
}

IMAGE_EXTS = {".png", ".jpg", ".jpeg", ".bmp", ".webp"}
IMAGE_DIRS = {
    "generated": ROOT / "datasets/_dvc_generated_combo/generated",
    "accepted": ROOT / "datasets/_dvc_generated_combo/accepted",
    "rejected": ROOT / "datasets/_dvc_generated_combo/rejected",
    "real_ab_fake": ROOT / "results/_gen_real_ab_reference_train_shampoo_or_shampoo_blade/fake_images",
}
...
def run_dvc(dvc_cmd: str, target: str | None = None, force: bool = False) -> None:
    ...
    with STATE.lock:
        ...
        STATE.append_log(f"[dashboard] dvc repro exited with code {returncode}")
        STATE.process = None
...
<head>
  ...
  <title>X-ray DVC Pipeline</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #111317;
...
<body>
  <header>
    <div>
      <h1>X-ray DVC Pipeline</h1>
      <div class="sub" id="subtitle">Idle</div>
    </div>
    <div class="controls">
      <button class="primary" id="startBtn">Start DVC</button>
  ...
  <script>
    ...
    function render(data) {
      ...
      document.getElementById('bar').style.width = `${Math.round((state.progress || 0) * 100)}%`;
...
class Handler(BaseHTTPRequestHandler):
    server_version = "XrayDvcDashboard/1.0"
    ...
    def do_GET(self) -> None:
        ...
        if path == "/":
            ...
            return
        if path == "/api/state":
            self.send_json({"state": STATE.snapshot(), "metrics": collect_metrics(), "images": list_gallery_images()})
            return
        if path == "/api/metrics":
    ...
    def do_POST(self) -> None:
        ...
        if path == "/api/start":
            ...
            thread = threading.Thread(target=run_dvc, args=(dvc_cmd, target, force), daemon=True)
            thread.start()
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run a live dashboard for the DVC automated pipeline.")
    ...
    print("[dashboard] click Start DVC to run the pipeline")
    httpd.serve_forever()
